...
async function getWeb01Db() {
  ...
  if (!count || count.c === 0) {
    ...
    web01Db.prepare(`INSERT INTO users (username,password,role,email) VALUES (?,?,?,?)`).run('alex','password123','user','alex@corp.internal');
    ...
    // FIX: pehle seed flag 'ACX{blind_injection_is_still_injection}' tha lekin
    // routes.js FLAG constant 'ACX{sql1_bypassed_auth_layer}' hai — dono alag the.
    // secret_data response mein ye DB wala flag dikhta tha jo user submit karta
    // toh 'Incorrect' aata tha. Ab dono consistent kar diye.
    web01Db.prepare(`INSERT INTO secrets (id,flag,note) VALUES (?,?,?)`).run(1,'ACX{sql1_bypassed_auth_layer}','Do not expose this table to the app layer');
...
// For web02 XSS
// FIX: pehle getWeb02Db() har call pe naya PersistentDB banata tha —
// writes ek instance mein jaate, reads alag instance se — comments
// persist nahi karte the reliably. Ab singleton cache karo.
let web02Db = null;
async function getWeb02Db() {
  if (web02Db) return web02Db;
  const SQL = await initSqlJs();
  const dbPath = path.join(DATA_DIR, 'web02.db');
  web02Db = new PersistentDB(dbPath);
  await web02Db.init(SQL);
  web02Db.exec(`
...
// For hidden-sight (flag submission tracking)
// FIX: same singleton pattern — pehle har call pe naya instance tha.
let hiddenDb = null;
async function getHiddenDb() {
  if (hiddenDb) return hiddenDb;
  const SQL = await initSqlJs();
  const dbPath = path.join(DATA_DIR, 'hidden.db');
  hiddenDb = new PersistentDB(dbPath);
  await hiddenDb.init(SQL);
  hiddenDb.exec(`
