...
const fs       = require('fs');
const { applyRouteSeo } = require('./seoMeta');

const app = express();

// index.html ek baar startup pe hi memory mein cache kar lo (388KB file
// hai, har SPA request pe disk se dobara padhna wasteful hai). Known
// /tutorials/* aur /tools/* routes ke liye ise route-specific title/
// description/canonical/schema ke saath serve kiya jaata hai (seoMeta.js
// dekho) - baaki routes plain cache hi paate hain, jaisa pehle tha.
const _indexHtmlCache = fs.readFileSync(path.join(__dirname, '../index.html'), 'utf8');
...
app.use('/api/lab/deserialize', require('./labs/web06-deserialize/routes'));
app.use('/api/lab/deserialize-medium', require('./labs/web07-deserialize-medium/routes'));
...
//
// FIX: pehle sab routes yahan LITERALLY same index.html paate the - koi
// bhi /tutorials/* ya /tools/* page apni alag <title>/description/schema
// crawler ko nahi deta tha (client-side router title/meta swap karta hai,
// lekin wo sirf JS chalne ke baad hota hai - non-JS crawlers, jisme kai
// AI search bots bhi shamil hain, sirf yahi original server response
// dekhte hain). applyRouteSeo() known routes ke liye sahi, alag tags
// splice kar deta hai; jo route map mein nahi hai use plain cache milta
// hai, jaisa pehle tha.
app.get([
  '/rooms', '/rooms/*',
  '/tutorials', '/tutorials/*',
  ...
  res.type('html').send(applyRouteSeo(_indexHtmlCache, req.path));
});
