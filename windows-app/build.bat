...
REM Always run from the folder this .bat file lives in, so it finds
REM requirements.txt / main.py even if double-clicked via a shortcut
REM or a copy that ended up in a different folder.
cd /d %~dp0
...
python -m PyInstaller --onefile --windowed --add-data "tray_icon.png;." --name AlexSync main.py
if errorlevel 1 (
    echo.
