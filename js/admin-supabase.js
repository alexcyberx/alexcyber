/* ═══════════════════════════════════════════════════════════════
   ...
   admin.js, the older mock/local-only version this replaced, was
   never loaded by panel-*.html and has been deleted.
═══════════════════════════════════════════════════════════════ */
...
async function admLoadRecentSignups() {
  ...
  try {
    // FIX AS-1: email not in profiles table, use username as secondary identifier
    ...
    el.innerHTML = data.map(u => {
      const name = escHtml(u.full_name || u.username || 'User');
      const sub  = u.username ? '@' + escHtml(u.username) : 'No username set';
...
function _admDrawSVG(data, lbls, mode) {
  ...
  // FIX AS-5: single data point pe path = "M x0 y0" only, no L commands.
  // area = "M x0 y0 L x0 H L x0 H Z", degenerate path, SVG may error.
  // Also: data.length === 0 → xs[0] undefined → crash.
  ...
  let path;
  if (data.length === 1) {
    // Single point: draw a dot instead of a line
    path = `M ${xs[0]} ${ys[0]}`;
  } else {
    path = `M ${xs[0]} ${ys[0]}`;
  ...
  const area = data.length > 1
    ? path + ` L ${xs[xs.length-1]} ${H} L ${xs[0]} ${H} Z`
    : `M ${xs[0]} ${H-pad} L ${xs[0]} ${H} Z`;
...
async function admLoadUsers() {
  ...
  try {
    // FIX AS-2: profiles table mein email column nahi hai, email auth.users mein hota hai.
    // .select('...email...') always returns null for email → '-' dikhta hamesha.
    // Fix: email column remove karo from select, username use karo as identifier.
    const { data, error } = await sb
      .from('profiles')
      .select('id, full_name, username, role, created_at, last_seen, is_banned, xp, level')
      .order('created_at', { ascending: false });
  ...
  try {
    ...
    el.innerHTML = `
      <table class="adm-table">
        <thead><tr><th>#</th><th>Player</th><th>XP</th><th>Level</th><th>Solves</th></tr></thead>
        <tbody>
          ${data.map((u, i) => {
...
async function admLoadGamification() {
  ...
  try {
    ...
    // NOTE: Badge aur streak system remove kar diya gaya hai (profile
    // page ab sirf Notifications + Settings tak simplify hai), isliye
    // badges/streak_current columns ab exist nahi karte.
    setText('gamBadgeCount', '-');
    setText('gamActiveStreaks', '-');
...
/* ════════════════════════════════════════
   MESSAGES
   Moved: real, wired-up implementation now lives directly in
   admin.html (renderMessages/openMessage/replyEmail/deleteMsg).
   This file's admNav wrapper below never actually fired for
   admin.html (which calls nav(), not admNav()), so this block
   was dead code, removed to avoid confusion.
════════════════════════════════════════ */
...
// FIX AS-4: pehle koi guard nahi tha, agar script reload ho (hot reload / SPA revisit)
// toh window.admNav apne aap ko wrap karta rahe → infinite call stack on nav click.
// Ab flag se ensure karo sirf ek baar wrap ho.
if (!window._admNavWrapped) {
  window._admNavWrapped = true;
